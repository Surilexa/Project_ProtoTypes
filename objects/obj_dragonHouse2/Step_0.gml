...
if(houseDone == 0)
{
	...
	else if (obj_TopDownPC.key_int1==1)  && (distance_to_object(obj_TopDownPC)<=range) && (distance_to_point(mouse_x,mouse_y)<=range && firstTime == 1)
	{
		...
		if(obj_TopDownPC.wood >= woodCost && buildHouse != 1 && obj_TopDownPC.rock >= rockCost)
		{
			buildHouse = 1;
			txtbox = 4;
			obj_TopDownPC.wood -= woodCost;
			obj_TopDownPC.rock -= rockCost;
			alarm[0] = 60 * 4;
		}
		...
		else if(obj_TopDownPC.wood < woodCost || obj_TopDownPC.rock < rockCost)
		{
			txtbox = 3;
		}
	}
