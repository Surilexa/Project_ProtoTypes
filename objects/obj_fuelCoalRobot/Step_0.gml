...
if (obj_pcController.key_int1_press==1)  && (distance_to_object(obj_pcController)<=range)
{
	if(txtbox == 1 or txtbox == 2)
	{
		...
		if(obj_pcCoalRobot.battery == obj_pcCoalRobot.battery_max)
		{
			txtbox = 4;
		}
		...
		else if(obj_RobotManager.coal_mined_total > 0)
		{
			if(obj_pcCoalRobot.battery < obj_pcCoalRobot.battery_max)
			{
				...
				energy_needed = obj_pcCoalRobot.battery_max - obj_pcCoalRobot.battery;
				//coal == 10 energy
				...
				if(coal_value > energy_needed)
				{
					...
					new_power = obj_pcCoalRobot.battery;
				}
				else if (coal_value < energy_needed)
				{
					obj_pcCoalRobot.battery += coal_value;
					new_power = obj_pcCoalRobot.battery;
				}
			}
			txtbox = 2;
