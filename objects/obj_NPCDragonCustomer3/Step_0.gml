...
if(obj_NPCDrinkShop.isBuilt && obj_PCunicorn.fishingBuff)
{
	fetchItemsArray = ["coconut", "pineapple", "watermelon", "goldfish","carp", "catfish","watermelonjuice", "pineapplejuice"];
}
else if(obj_NPCDrinkShop.isBuilt)
{
	fetchItemsArray = ["coconut", "pineapple", "watermelon", "goldfish","carp","watermelonjuice", "pineapplejuice"];
}
else if(obj_PCunicorn.fishingBuff)
{
	fetchItemsArray = ["coconut", "pineapple", "watermelon", "goldfish","carp","catfish"];
}
...
if (obj_PCunicorn.key_int1==1)  && (distance_to_object(obj_PCunicorn)<=range) && (distance_to_point(mouse_x,mouse_y)<=range)
{
	...
	if (givenQuest == false)
	{
		...
		{
			fetchItemAmount = irandom_range(3, 5);
			questReward = fetchItemAmount * coconutCost;
		}
		else if(fetchItem == "pineapple")
		{
			fetchItemAmount = irandom_range(6, 8);
