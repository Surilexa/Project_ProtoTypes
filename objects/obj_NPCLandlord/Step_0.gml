...
if (obj_pcHorse.key_int1==1)  && (distance_to_object(obj_pcHorse)<=range) && (distance_to_point(mouse_x,mouse_y)<=range)
{
	...
	if (givenQuest == false)
	{
		...
		if(fetchItem == "Rent")
		{
			fetchItemAmount = irandom_range(100, 125);
			//fishing buff?
		}
	...
	else if(givenQuest == true && fetchItem == "Rent" && obj_pcHorse.money >= fetchItemAmount)
	{
		txtbox = 2;
		//take the required resource away from player
		obj_pcHorse.money -= fetchItemAmount;
		
		obj_pcHorse.fishingBuff = true;
